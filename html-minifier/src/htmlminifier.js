...
(function(global){
  ...
  function minify(value, options) {
    ...
    var results = [ ],
        ...
        lint = options.lint,
        handleScript = options.handleScript || false,
        handleStyle = options.handleStyle || false,
        t = new Date()
    
    HTMLParser(value, {
      options: {
        skipTemplateTags: options.skipTemplateTags || false
      },
      ...
      chars: function( text ) {
        if (currentTag === 'script' || currentTag === 'style') {
          ...
          if (handleStyle && currentTag === 'style') {
            text = handleStyle(text);
          }
          if (handleScript && currentTag === 'script') {
            text = handleScript(text);
          }
        }
        if (options.collapseWhitespace) {
