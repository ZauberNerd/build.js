    ...
    while ( html ) {
      ...
      if ( !stack.last() || !special[ stack.last() ] ) {
        ...
        if ( chars ) {
          ...
          if (handler.options && handler.options.skipTemplateTags) {
            var skipSpecialTags = handler.options.skipTemplateTags;
            for (var i = 0; i < skipSpecialTags.length; i++) {
              if (html.indexOf(skipSpecialTags[i][0]) !== -1){
                index = html.indexOf(skipSpecialTags[i][1]) + 2;
              }              
            }
          }
